...
namespace chat_notify_types
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            StreamWriter sw = new StreamWriter("ChatNotify.h");

            sw.WriteLine("enum ChatNotify");
            sw.WriteLine("{");

            foreach (string str in names)
            {
                string temp = string.Format("CHAT_{0}_NOTICE",str);
        ...
        {
            int pos = stream_string.IndexOf("VOICE_OFF");
            return pos;
        }
